...
> **Status (Bloco 8 — 2026-05-21):** REGRA APLICADA GLOBALMENTE. Os tokens `--np-container-max` (variables.css), `--page-max` (home-v2/tokens.css), `maxWidth.np-container` (tailwind-config.js) e o divider de home-v2/base.css foram migrados para **1280px**. O `.footer__container` padding lateral migrou de 40 → 32. A referência operacional aprovada é a página `/games` após Blocos 6 e 7.
